...
from utils import pair_round, random_pairs
...
class BaseMelodyGenerator:
    ...
    def generate_random_melodies(self) -> dict:
        ...
        for i in range(self.n_melodies):
            ...
            durations = random.choices([240, 480, 960, 1920], k=self.n_melody_notes)
            melodies[i] = {"notes": notes, "durations": durations}
        return melodies
    
    def save_melodies(self, save_path: str = "melodies.json") -> None:
        """
        Saves the generated melodies to MIDI files and a JSON file.

        :param save_path: The file path where melodies will be saved in JSON format.
        """
...
class SimulatedAnnealingGA(BaseMelodyGenerator):
    ...
    def __init__(
        ...
        """
        super().__init__(n_melodies, n_melody_notes, heuristic)
